  ...
  , m_mainWindow(mainWindow)
{
  ...
  m_dirOperator->setOnlyDoubleClickSelectsFiles(true);
...
void KateFileBrowser::fileSelected(const KFileItem & /*file*/)
{
  ...
  std::cout << "CYCKI: Wczytywanie pliku..." << std::endl;
  if(list.count() == 1){
    if(list.first().isDir()){
      if(last == &list.first()){
        openSelectedFiles();
        std::cout << "CYCKI: katalog: Otwieranie..." << std::endl;
      }else{
        std::cout << "CYCKI: katalog: Pierwsze klikniecie" << std::endl;
        last = &list.first();
      }
    }else{
      std::cout << "CYCKI: plik: otwieranie" << std::endl;
      openSelectedFiles();
    }
  }
  openSelectedFiles();
